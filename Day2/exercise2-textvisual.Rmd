...
```{r}
plot(tfidf(dfm), scale=c(3.5, .75), colors=brewer.pal(8, "Dark2"), 
     random.order = F, rot.per=0.1, max.words=100)
```
...
dfm <- dfm(MyCorpus, groups = "Candidate",
           ignoredFeatures = c(stopwords("english"), "t.co", "https", "rt", "amp", "http", "t.c", "can", "u"),
           ngrams=c(1,2))
```
...
```{r}
plot(dfm, comparison = T, scale=c(3.5, .75), colors=brewer.pal(8, "Dark2"), 
     random.order = F, rot.per=0.1, max.words=100)
```
